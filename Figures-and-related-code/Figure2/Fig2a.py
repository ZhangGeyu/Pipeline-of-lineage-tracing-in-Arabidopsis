...
Progeny_mutation = pd.read_table('MutFreq_In_ProgenySample_Plant1.txt')
Progeny_mutation = Progeny_mutation[(Progeny_mutation['mut_freq'] >= 0.5)&(~Progeny_mutation['mut_info'].isin(Hotspot_Plant1))] # remove hotspot mutations and keep records with mutation frequency ≥0.5
# Extract position, reference base, alternative base from mutation info
...
for SampleName in Parental_mutation['SampleName'].unique():
    ...
    if len(SampleName_UMI_list) >= 50:
        # Randomly select 50, put unselected UMIs into Not_SampleName_UMI_list
...
with open((path + 'ConsensusSeq_Plant1.fasta'), 'w') as fasta_consensus:
