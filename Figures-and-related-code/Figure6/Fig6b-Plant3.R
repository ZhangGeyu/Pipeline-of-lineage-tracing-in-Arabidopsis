...
PassToOffspring_df <- read.table('Mut_PassToOffspring_BranchShared-Plant3.txt', header = TRUE)
...
Parent_MutCount_bar <- ggplot(PassToOffspring_df,
  ...
  scale_y_continuous(breaks = seq(0, 10000, by = 200)) + xlab('') + 
...
PassToOffspring_MutCount_bar <- ggplot(PassToOffspring_df,
  ...
  geom_bar(stat = 'identity',colour = 'black') + 
  scale_y_continuous(breaks = seq(0, 500, by = 25)) + xlab('') + 
  scale_fill_manual(values = c('#ade8f4','#48cae4','#0096c7','#0077b6')) + 
  Figure_Theme +
  theme(axis.text.x = element_blank(),
        legend.position="none")
