...
### Pair Branch two by two for random sampling
Hotspot_Plant1 = ["1092_T_-1C","1114_A_G","1226_C_T","1335_C_T","445_C_T","513_C_G","53_C_-2GA","77_G_A","795_G_T","965_G_A"] # Remove hotspot mutation
...
for Branch in Branch_list:
    ...
    locals()['Offspring_Mut' + Branch] = locals()['Offspring_Mut' + Branch][locals()['Offspring_Mut' + Branch]['mut_info'].isin(OP_Mut_list)]
...
Branch_combinations = list(combinations(Branch_list, 2))

# Clean file
with open('BranchCombination_RandomSampling.txt','w') as MutRandom:
    MutRandom.write('Branch\t'+'Group\t'+'Mut_InDiffBranch_Count\n')

    for Branch_selected in Branch_combinations:
        Branch_selected_str = str(Branch_selected).replace("', '","_").replace("('",'').replace("')",'')

        with open(('BranchCombination_' + Branch_selected_str + '_UMI_MutCount.txt'),'w') as UMI_MutCount:
            UMI_MutCount.write('mut_count\t'+'Group\t'+'Branch\t'+'num\n')

            Offspring_Mut_Branch_1 = Offspring_Mut[Offspring_Mut['Branch'] == Branch_selected[0]]
            Offspring_Mut_Branch_2 = Offspring_Mut[Offspring_Mut['Branch'] == Branch_selected[1]]
            MutOffspring_Mut_BranchCount = len(set(Offspring_Mut_Branch_1['mut_info'].unique()) & set(Offspring_Mut_Branch_2['mut_info'].unique()))
            ...
            Offspring_UMI_MutCount_1['Branch'] = 'Branch_' + Branch_selected[0]
            ...
            Offspring_UMI_MutCount_2['Branch'] = 'Branch_' + Branch_selected[1]
            ...
            for i in range(1,n+1):
                ...
                Parental_Mut_Branch_1 = locals()['Parental_Mut_UMI' + Branch_selected[0]]
                Parental_Mut_Branch_2 = locals()['Parental_Mut_UMI' + Branch_selected[1]]
                Mut_InDiffBranch_Count = len(set(Parental_Mut_Branch_1['mut_info'].unique()) & set(Parental_Mut_Branch_2['mut_info'].unique()))

                MutRandom.write(Branch_selected_str + '\t' + 'Random' + '\t' + str(Mut_InDiffBranch_Count) + '\n')
            MutRandom.write(Branch_selected_str + '\t' + 'Observation' + '\t' + str(MutOffspring_Mut_BranchCount) + '\n')
