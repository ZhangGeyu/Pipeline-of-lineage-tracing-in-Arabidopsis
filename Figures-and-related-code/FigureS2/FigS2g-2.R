...
## S-M test for Other Progeny

library(phangorn)

tree <- read.tree('ConsensusSeq_Tree_Plant1.nwk')    # Read tree file in Newick format
...
traits$trait_value <- ifelse(grepl('P', traits$species) & !traits$species %in% Offspring_1, 1, 0)
...
S_M_test <- ggplot(df_score, aes(x = score)) +
  geom_histogram(bins = 4, fill = "grey", color = "black") +
  geom_vline(xintercept = obs_score, color = "red", linetype = "dashed", size = 0.5) +
  labs(title = "Plant1 Other Progeny",x = "Parsimony Score",y = "Count") +
  scale_x_continuous(breaks = seq(0, 1000, by = 1)) + 
  scale_y_continuous(breaks = seq(0, 1000, by = 200)) + 
  annotate("text", x = obs_score - 2, y = max(table(null_scores)) - 100,
