...
# Remove hotspot mutations and copy mutations
...
# Offspring data
# Offspring haplotype is generated by code from figS3e, see in Pipeline-of-lineage-tracing-in-Arabidopsis/Data
Progeny_mutation = pd.read_table('Offspring_haplotype-Plant3.txt')
Progeny_mutation = Progeny_mutation[(~Progeny_mutation['mut_info'].isin(Hotspot_Plant3))&(~Progeny_mutation['mut_info'].isin(HighFreqMut_list_181copy))]
...
# Randomly select UMIs for each parental sample, but use different sampling numbers for different branches
for SampleName in Parental_mutation['SampleName'].unique():
    ...
    if SampleName[0] == 'R':
        if len(SampleName_UMI_list) >= 40:
            Not_SampleName_UMI_list = set(SampleName_UMI_list) - set(random.sample(SampleName_UMI_list,40)) 
            Parental_mutation = Parental_mutation[~Parental_mutation['SampleName_UMI'].isin(Not_SampleName_UMI_list)]
        else:
            continue
    elif SampleName.split('-')[0] == 'B1':
        if len(SampleName_UMI_list) >= 20:
            Not_SampleName_UMI_list = set(SampleName_UMI_list) - set(random.sample(SampleName_UMI_list,20)) 
            Parental_mutation = Parental_mutation[~Parental_mutation['SampleName_UMI'].isin(Not_SampleName_UMI_list)]
        else:
            continue
    elif SampleName.split('-')[0] == 'B2':
        if len(SampleName_UMI_list) >= 40:
            Not_SampleName_UMI_list = set(SampleName_UMI_list) - set(random.sample(SampleName_UMI_list,40)) 
            Parental_mutation = Parental_mutation[~Parental_mutation['SampleName_UMI'].isin(Not_SampleName_UMI_list)]
        else:
            continue
    elif SampleName.split('-')[0] == 'B3':
        if len(SampleName_UMI_list) >= 100:
            Not_SampleName_UMI_list = set(SampleName_UMI_list) - set(random.sample(SampleName_UMI_list,100))
            Parental_mutation = Parental_mutation[~Parental_mutation['SampleName_UMI'].isin(Not_SampleName_UMI_list)]
        else:
            continue
...
reference = SeqIO.read('reference-Plant3.fa', "fasta")
...
with open('ConsensusSeq_Plant3.fasta', 'w') as fasta_consensus:
