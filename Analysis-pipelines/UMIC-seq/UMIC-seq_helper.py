...
def select_bc(scores):
    ...
    #if sorted_scores[0] > 12 and sorted_scores[1] <= 10:
    if sorted_scores[0] >= 15: #highthresh and sorted_scores[1] <= lowthresh:
        return bc_id
    else:
        return n_barcodes
...
if mode == 'demultiplex':
    ...
    N_seq = len([1 for line in open(input_file)]) // 2 #4
    ...
    for bc_name in barcode_names:
        filename = f'{output_name}_{bc_name}.fasta'
        h = open(filename, "w")
        handles.append(h)

    i = 0
    for rec in SeqIO.parse(input_file, 'fasta'):
        txt = rec.format("fasta")
